...
minetest.register_alias("mapgen_sapling", "default:sapling")
minetest.register_alias("mapgen_leaves", "default:leaves")
minetest.register_alias("mapgen_jungletree", "default:jungletree")
minetest.register_alias("mapgen_jsapling", "default:junglesapling")
...
function default.generate_ore(name, wherein, minp, maxp, seed, chunks_per_volume, chunk_size, ore_per_chunk, height_min, height_max)
	...
	for i=1,num_chunks do
		local y0 = math.random(y_min, y_max-chunk_size+1)
		if y0 >= height_min and y0 <= height_max then
			local x0 = math.random(minp.x, maxp.x-chunk_size+1)
			local z0 = math.random(minp.z, maxp.z-chunk_size+1)
			local p0 = {x=x0, y=y0, z=z0}
			for x1=0,chunk_size-1 do
			for y1=0,chunk_size-1 do
			for z1=0,chunk_size-1 do
				if math.random(1,inverse_chance) == 1 then
					local x2 = x0+x1
					local y2 = y0+y1
					local z2 = z0+z1
...
function generate_nyancats(seed, minp, maxp)
	...
	for i=1,max_num_nyancats do
		if math.random(0, 1000) == 0 then
			...
			local p0 = {x=x0, y=y0, z=z0}
			default.make_nyancat(p0, math.random(0,3), math.random(3,15))
		end
	end
end
		...
		for divz=0,divs-1 do
			...
			for i=0,papyrus_amount do
				...
				if minetest.get_node({x=x,y=1,z=z}).name == "default:dirt_with_grass" and
					...
					default.make_papyrus({x=x,y=2,z=z}, math.random(2, 4))
				end
			end
		end
		end
		...
		for divz=0,divs-1 do
			...
			for i=0,cactus_amount do
				...
				if ground_y and minetest.get_node({x=x,y=ground_y,z=z}).name == "default:desert_sand" then
					default.make_cactus({x=x,y=ground_y+1,z=z}, math.random(6, 8))
				end
			end
		end
		end
				...
				if ground_y then
					...
					if minetest.registered_nodes[nn] and
						...
						elseif nn == "default:dirt_with_grass" then
							minetest.set_node(p,{name="default:grass_"..math.random(1, 5)})
						end
					end
				end
