...
function default.grow_tree(data, a, pos, is_apple_tree, seed)
    ...
    local hight = math.random(8, 14)
    if is_apple_tree == true then hight = 5 end  
    for x_area = -3, 3 do
    for y_area = -2, 3 do
    for z_area = -3, 3 do
        if math.random(1,30) < 23 and math.abs(x_area) + math.abs(z_area) < 5 and math.abs(x_area) + math.abs(y_area) < 5 and math.abs(z_area) + math.abs(y_area) < 5 then  --randomize leaves
            local area_l = a:index(pos.x+x_area, pos.y+hight+y_area-1, pos.z+z_area)  --sets area for leaves
            if data[area_l] == c_air or data[area_l] == c_ignore then    --sets if air or ignore
                if is_apple_tree and math.random(1, 50) <=  10 then  --randomize apples
...
function default.grow_jungletree(data, a, pos, seed)
    ...
    for y_area = -3, 4 do
    for z_area = -5, 5 do
        if math.random(1,30) < 23 and math.abs(x_area) + math.abs(z_area) < 8  and math.abs(x_area) + math.abs(y_area) < 8 and math.abs(z_area) + math.abs(y_area) < 8 then  --randomize leaves
            local area_l = a:index(pos.x+x_area, pos.y+hight+y_area-1, pos.z+z_area)  --sets area for leaves
            if data[area_l] == c_air or data[area_l] == c_ignore then    --sets if air or ignore
